...
void HighsDomain::updateActivityLbChange(HighsInt col, double oldbound,
  ...
  for (HighsInt i = start; i != end; ++i) {
    const HighsInt iRow = mip->a_matrix_.index_[i];
    ...
    if (mip->a_matrix_.value_[i] > 0) {
      ...
      if (inProbing_ && (mipsolver->options_mip_->DFProbing || mipsolver->options_mip_->GDF) && activitymininf_[iRow] == 0 && 
          !redundantPropagateflags_[2 * iRow] && mipsolver->model_->row_lower_[iRow] != -kHighsInf &&
          newbound >= oldbound + mipsolver->mipdata_->feastol) {
        double actmin = (double)activitymin_[iRow];
        if (actmin >= mipsolver->model_->row_lower_[iRow] - mipsolver->mipdata_->feastol)
          markRedundantPropagate(iRow, false);
      }
      ...
      if (inProbing_ && (mipsolver->options_mip_->DFProbing || mipsolver->options_mip_->GDF) && activitymaxinf_[iRow] == 0 &&
         !redundantPropagateflags_[2 * iRow + 1] && mipsolver->model_->row_upper_[iRow] != kHighsInf &&
          newbound >= oldbound + mipsolver->mipdata_->feastol) {
        double actmax = (double)activitymax_[iRow];
        if (actmax <= mipsolver->model_->row_upper_[iRow] + mipsolver->mipdata_->feastol)
          markRedundantPropagate(iRow, true);
      }
  ...
  if (infeasible_) {
    ...
    for (HighsInt i = start; i != end; ++i) {
      if (mip->a_matrix_.value_[i] > 0) {
        ...
        if (inProbing_ && (mipsolver->options_mip_->DFProbing || mipsolver->options_mip_->GDF))
          cancelRedundantPropagate(mip->a_matrix_.index_[i], false);
      } else {
        activitymax_[mip->a_matrix_.index_[i]] +=
            computeDelta(mip->a_matrix_.value_[i], oldbound, newbound,
                         -kHighsInf, activitymaxinf_[mip->a_matrix_.index_[i]]);
        if (inProbing_ && (mipsolver->options_mip_->DFProbing || mipsolver->options_mip_->GDF))
          cancelRedundantPropagate(mip->a_matrix_.index_[i], true);
      }
    }
...
void HighsDomain::cancelRedundantPropagate(HighsInt row, bool isUpper) {
  assert(row < (int)mipsolver->numRow());
  const HighsInt pos = 2 * row + isUpper;

  auto removePos = [&](HighsInt pos, vector<HighsInt>& v) {
    // remove pos in a vector
    HighsInt i;
    ...
    if (v[i] != pos)
      std::cout << "remove vector work abnormal!\n";
    v[i] = v.back();
    v.pop_back();
  };
  ...
  if (redundantPropagateflags_[pos]) {
    if (isUpper) {
      ...
      if (!upperRedundant) {
        removePos(pos, redundantPropagateinds_);
        redundantPropagateflags_[pos] = 0;
        if (mipsolver->options_mip_->GDF) {
          if (firstOccuranceFlag_[pos] && inProbing_ == 0) {
            removePos(pos, twoSideRedundantInds_);
            // auto it = twoSideRedundantInds_.find(pos);
            // if (it != twoSideRedundantInds_.end())
            //   twoSideRedundantInds_.erase(it);
          }
          firstOccuranceFlag_[pos] = false;
        }
      }
    }
    else {
...
bool HighsDomain::isUpperRedundant(HighsInt row) {
  bool upperRedundant;
  
  upperRedundant = (mipsolver->model_->row_upper_[row] != kHighsInf) &&
    (getMaxActivity(row) <= mipsolver->model_->row_upper_[row] + mipsolver->mipdata_->feastol);
  return upperRedundant;
}

bool HighsDomain::isLowerRedundant(HighsInt row) {
  bool lowerRedundant;

  lowerRedundant = (mipsolver->model_->row_lower_[row] != -kHighsInf) &&
      (getMinActivity(row) >= mipsolver->model_->row_lower_[row] - mipsolver->mipdata_->feastol);
  return lowerRedundant;
}

void HighsDomain::markRedundantPropagate(HighsInt row, bool isUpper) {
  assert(row < (int)mipsolver->numRow());
  if (mipsolver->submip)
    return;
  const HighsInt pos = 2 * row + isUpper;
  if (!redundantPropagateflags_[pos]) {
    if (isUpper) {
      ...
      if (upperRedundant) {
        ...
        if (mipsolver->options_mip_->GDF && !startZeroCostFixing) {
          if (probingStatusSide == 1) {
            firstOccuranceInds_.push_back(pos);
            firstOccuranceFlag_[pos] = true;
          }
          else if (probingStatusSide == 0) {
            if (firstOccuranceFlag_[pos])
              twoSideRedundantInds_.push_back(pos);
          }
          else {
            assert(probingStatusSide == -1); // do nothing
          }
        }
      }
    }
...
void HighsDomain::probingVariableGDF(HighsInt col, bool val) {
  if (mipsolver->submip)
    return;
  auto mip = mipsolver->model_;
  HighsInt foundHere = 0;
  if (val) { // x_k = 1， consider lower lock
    if (mipsolver->model_->colLoLockNum[col] > mipsolver->numRow())
      ;
    else {
      // For constraints in the form of \sum_{j \in \N} a_j x_j \le b_{iRow} with a_{col} < 0:
      //    Condition for lower bound reachable: o_{col}^{-, iRow} + 0 \le b_{iRow}
      //    Approximation of o_{col}^{-, iRow}: o_{col}^{-, iRow} \le \max{\sum_{j \in \N \backslash \{col\}} a_j x_j | x_{col} = 1}
      //                                                           =  getmax(iRow | x_{col} = 1) - a_{col} * 1
      //    So the condition becomes:  getmax(iRow | x_{col} = 1) - a_{col} \le b_{iRow}
      //
      // For constraints in the form of \sum_{j \in \N} a_j x_j \ge b_{iRow} with a_{col} > 0, they can be transformed to \le form by multiplying -1 in both sides:
      //  \sum_{j \in \N} (-a_j) x_j \le (-b_{iRow}):
      //    The condition for lower bound reachable can be derived similarly: - getmin(iRow | x_{col} = 1) + a_{col} \le (-b_{iRow}) <=> getmin(iRow | x_{col} = 1) - a_{col} \ge b_{iRow}
      HighsInt start = mip->a_matrix_.start_[col];
      HighsInt end = mip->a_matrix_.start_[col + 1];
      for (HighsInt i = start; i != end; ++i) {
        const HighsInt iRow = mip->a_matrix_.index_[i];
        const double v = mip->a_matrix_.value_[i];
        const bool isLhs = redundantPropagateflags_[2 * iRow];
        const bool isRhs = redundantPropagateflags_[2 * iRow + 1];
        if (v < 0 && isRhs) {
          if (getMaxActivity(iRow) - v <= mip->row_upper_[iRow] + feastol())
            permanentLowerLock_[col].insert(2 * iRow + 1), foundHere ++;
        }
        else if (v > 0 && isLhs) {
          if (getMinActivity(iRow) - v >= mip->row_lower_[iRow] - feastol())
            permanentLowerLock_[col].insert(2 * iRow), foundHere ++;
        }
      }
    }
  }
  else { // x_k = 0， consider upper lock
    if (mipsolver->model_->colUpLockNum[col] > mipsolver->numRow())
      ;
    else {
      // For constraints in the form of \sum_{j \in \N} a_j x_j \le b_{iRow} with a_{col} > 0:
      //    Condition for upper bound reachable: o_{col}^{+, iRow} + a_j \le b_{iRow}
      //    Approximation of o_{col}^{+, iRow}: o_{col}^{+, iRow} \le \max{\sum_{j \in \N \backslash \{col\}} a_j x_j | x_{col} = 0}
      //                                                           = getmax(iRow | x_{col} = 0)
      //    So the condition becomes:  getmax(iRow | x_{col} = 0) + a_{col} \le b_{iRow}
      //
      // For constraints in the form of \sum_{j \in \N} a_j x_j \ge b_{iRow} with a_{col} < 0, they can be transformed to \le form by multiplying -1 in both sides:
      //  \sum_{j \in \N} (-a_j) x_j \le (-b_{iRow}):
      //    The condition for upper bound reachable can be derived similarly: - getmin(iRow | x_{col} = 0) - a_{col} \le (-b_{iRow}) <=> getmin(iRow | x_{col} = 0) + a_{col} \ge b_{iRow}
      HighsInt start = mip->a_matrix_.start_[col];
      HighsInt end = mip->a_matrix_.start_[col + 1];
      for (HighsInt i = start; i != end; ++i) {
        const HighsInt iRow = mip->a_matrix_.index_[i];
        ...
        if (v > 0 && isRhs) {
          if (getMaxActivity(iRow) + v <= mip->row_upper_[iRow] + feastol())
            permanentUpperLock_[col].insert(2 * iRow + 1), foundHere ++;
        }
        else if (v < 0 && isLhs) {
          if (getMinActivity(iRow) + v >= mip->row_lower_[iRow] - feastol())
            permanentUpperLock_[col].insert(2 * iRow), foundHere ++;
        }
      }
    }
  }
...
void HighsDomain::updateGDFInfo() {
  // if (!twoSideRedundantInds_.empty()) {
  //   std::cout << "two side redundant index num : " << twoSideRedundantInds_.size() << std::endl;
  // }
  if (mipsolver->submip)
    return;
  const auto& implications = mipsolver->mipdata_->implications;
  if (mipsolver->options_mip_->GDF) {
      for (auto r : twoSideRedundantInds_) {
        const HighsInt iRow = r / 2;
        const bool isRhs = r % 2;
        assert(redundantAbsResidue_[r] != kHighsInf);
        ...
        for (auto k = rstart; k < rend; k++) {
          ...
          if (col_lower_[iCol] == -kHighsInf || col_upper_[iCol] == kHighsInf || implications.colsubstituted[iCol])
            continue;

          if (cost <= mipsolver->options_mip_->dual_feasibility_tolerance) { // consider upper lock
            if (mipsolver->model_->colUpLockNum[iCol] > mipsolver->numRow())
              continue;
            const double res = fabs((col_upper_[iCol] - tightestColUpper[iCol]) * iValue);
            if (isRhs && iValue > 0) {
              if (!varTightenFlag_[iCol] || res <= std::max(redundantAbsResidue_[r], 0.0))
                permanentUpperLock_[iCol].insert(r);
            }
            else if (!isRhs && iValue < 0) {
              if (!varTightenFlag_[iCol] || res <= std::max(redundantAbsResidue_[r], 0.0))
                permanentUpperLock_[iCol].insert(r);
            }
          }

          if (cost >= -mipsolver->options_mip_->dual_feasibility_tolerance) { // consider lower lock
            if (mipsolver->model_->colLoLockNum[iCol] > mipsolver->numRow())
              continue;
            const double res = fabs((col_lower_[iCol] - tightestColLower[iCol]) * iValue);
            if (!isRhs && iValue > 0) {
              if (!varTightenFlag_[iCol] || res <= std::max(redundantAbsResidue_[r], 0.0))
                permanentLowerLock_[iCol].insert(r);
            }
            else if (isRhs && iValue < 0) {
              if (!varTightenFlag_[iCol] || res <= std::max(redundantAbsResidue_[r], 0.0))
                permanentLowerLock_[iCol].insert(r);
...
int HighsDomain::finalRoundDualFixing() {
  int nfix = 0;
  auto mip = mipsolver->model_;
  for (HighsInt iCol = 0; iCol < mipsolver->numCol(); iCol ++) {
    if (isFixed(iCol))
      continue;
    if (mipsolver->mipdata_->implications.colsubstituted[iCol])
      continue;
    // if (nfix >= mipsolver->options_mip_->ipm_iteration_limit)
    //   break;
    // consider those variables that can not be fixed at lower < fabs(cost) <= eps >
    // upper lock
    if (mipsolver->colCost(iCol) <= mipsolver->options_mip_->dual_feasibility_tolerance && col_upper_[iCol] != kHighsInf) {
      bool canBeFixed = true;
      for (int k = mip->a_matrix_.start_[iCol]; k < mip->a_matrix_.start_[iCol+1]; k ++) {
        const HighsInt iRow = mip->a_matrix_.index_[k];
        const double v = mip->a_matrix_.value_[k];
        bool isTrueLock;
        HighsInt r = 2 * iRow;
        if (v > 0) {
          isTrueLock = (mipsolver->rowUpper(iRow) < getMaxActivity(iRow) + feastol());
          r ++;
        }
        else
          isTrueLock = (mipsolver->rowLower(iRow) > getMinActivity(iRow) - feastol());

        if (isTrueLock) {
          // this lock can not be removed
          auto it = permanentUpperLock_.find(iCol);
          if (it == permanentUpperLock_.end()) {
            canBeFixed = false;
            break;
          }
          else if (it->second.find(r) == permanentUpperLock_[iCol].end()) {
            canBeFixed = false;
            break;
          }
        }
      ...
      if (canBeFixed) {
        HighsDomainChange bchg;
        bchg.boundval = col_upper_[iCol];
        bchg.column = iCol;
        bchg.boundtype = HighsBoundType::kLower;
        changeBound(bchg);
        nfix ++;
        if (cumulateReduction())
          break;
      }
    }

    // 2. consider lower lock 
    if (!isFixed(iCol) && mipsolver->colCost(iCol) >= -mipsolver->options_mip_->dual_feasibility_tolerance && col_lower_[iCol] != -kHighsInf) {
      bool canBeFixed = true;
      for (int k = mip->a_matrix_.start_[iCol]; k < mip->a_matrix_.start_[iCol+1]; k ++) {
        const HighsInt iRow = mip->a_matrix_.index_[k];
        const double v = mip->a_matrix_.value_[k];
        ...
        if (isTrueLock) {
          // this lock can not be removed
          auto it = permanentLowerLock_.find(iCol);
          if (it == permanentLowerLock_.end()) {
            canBeFixed = false;
            break;
          }
          else if (it->second.find(r) == permanentLowerLock_[iCol].end()) {
            canBeFixed = false;
            break;
          }
        }
      ...
      if (canBeFixed) {
        HighsDomainChange bchg;
        bchg.boundval = col_lower_[iCol];
        bchg.column = iCol;
        bchg.boundtype = HighsBoundType::kUpper;
        changeBound(bchg);
        nfix ++;
        if (cumulateReduction())
          break;
...
bool HighsDomain::cumulateReduction() {
  mipsolver->mipdata_->num_reduction_limit ++;
  return mipsolver->mipdata_->num_reduction_limit >= mipsolver->options_mip_->ipm_iteration_limit;
}
...
void HighsDomain::initializeDualfixingUtils() {
  ...
  if (mipsolver->options_mip_->GDF) {
    ...
    firstOccuranceInds_.reserve(2 * mipsolver->numRow());
    firstOccuranceFlag_.assign(2 * mipsolver->numRow(), false);
    twoSideRedundantInds_.clear();
    twoSideRedundantInds_.reserve(2 * mipsolver->numRow());
    
    varTightenFlag_.assign(mipsolver->numCol(), false);
    varTightenInds_.clear();
    varTightenInds_.reserve(mipsolver->numCol());
    
    tightestColLower = col_lower_;
    tightestColUpper = col_upper_;
    redundantResidueflags_.assign(2 * mipsolver->numRow(), false);
    redundantResidueinds_.clear();
    redundantResidueinds_.reserve(2 * mipsolver->numRow());
    redundantAbsResidue_.assign(2 * mipsolver->numRow(), kHighsInf);
  }
}
...
double HighsDomain::doChangeBound(const HighsDomainChange& boundchg) {
  ...
  if (inProbing_ && isBchgTightenType(boundchg) && mipsolver->options_mip_->GDF && !startZeroCostFixing) {
    insertVarsBchg(boundchg);
  }
...
void HighsDomain::clearRedundant() {
  ...
  for (int i = 0; i < (int)redundantPropagateflags_.size(); i ++)
    assert(!redundantPropagateflags_[i]);
  
  zeroCostFixedVariables_.clear();
}

void HighsDomain::frozenRedundantResidue() {
  if (!redundantPropagateinds_.empty() && mipsolver->options_mip_->GDF) {
    for (auto x : redundantPropagateinds_) {
      const HighsInt iRow = x / 2;
      const bool isRhs = x % 2;
      const double residue = isRhs ? mipsolver->rowUpper(iRow) - getMaxActivity(iRow) :
                                     getMinActivity(iRow) - mipsolver->rowLower(iRow);
      if (residue >= - mipsolver->mipdata_->feastol) {
        redundantAbsResidue_[x] = std::min(redundantAbsResidue_[x], residue);
        if (!redundantResidueflags_[x]) {
          redundantResidueflags_[x] = true;
          redundantResidueinds_.push_back(x);
        }
      }
    }
  }
...
void HighsDomain::clearRedundantResidue() {
  if (!redundantResidueinds_.empty()) {
    for (auto x : redundantResidueinds_) {
      redundantResidueflags_[x] = false;
      redundantAbsResidue_[x] = kHighsInf;
    }
  }
  redundantResidueinds_.clear();
}
...
bool HighsDomain::propagate(bool dualflag) {
  std::vector<HighsInt> propagateinds;

  HighsInt countRedundantRow = 0;

  // The boolean variable ``startZeroCostFixing'' is used to flag if we allow variables with zero cost objective coefficients can be fixed in domain propagation.
  // The process of domain propagtion in probing is executed in two phases:
  //     Phase 1: Apply classic domain propagation, and additionally fix variables with non-zero objective coefficients using dual fixing
  ...
  // In Phase 1, ``startZeroCostFixing'' is set to be ``false'' to exclude variable with zero objective coefficients.
  // In Phase 2, ``startZeroCostFixing'' is set to be ``true''.
  // Note that
  ...
  startZeroCostFixing = false;

  auto haveRedundantPropagationRows = [&]() {
    if (!redundantPropagateinds_.empty() && dualflag)
      return true;
    else
      return false;
  };

  auto haveIncreasedRedundantPropagationRows = [&]() {
    if (!mipsolver->submip && countRedundantRow < (HighsInt)redundantPropagateinds_.size() && dualflag && mipsolver->options_mip_->DFProbing)
      return true;
    return false;
  };

  auto updateRedundantPropagation = [&](int pos) {
    // std::cout << pos << std::endl;
    const HighsInt oldPos = pos;
    vector<HighsInt>& collowerlock = tmpColLoLock_;
    vector<HighsInt>& colupperlock = tmpColUpLock_;
    // if (collowerlock.empty()) {
    //   std::cout << "AAAAAAAAAAAA\n";
    //   std::cout << HIGHS_PROBING_COUNT << std::endl;
    //   exit(1);
    // }

    assert(involvedVars.empty());

    auto insertElement = [&](HighsInt k) {
      // std::cout << "k = " << k << std::endl;
      if (indsVars[k])
        return;
      else {
        involvedVars.push_back(k);
        indsVars[k] = true;
      }
    };

    // get candidate
    HighsInt maxLockLeft = (HighsInt)redundantPropagateinds_.size() - pos;
    for (; pos < (HighsInt)redundantPropagateinds_.size(); pos ++, maxLockLeft --) {
      const HighsInt i = redundantPropagateinds_[pos];
      const HighsInt iRow = i / 2;
      assert(mipsolver->numRow() == (HighsInt)mipsolver->model_->row_lower_.size());
      assert(iRow < mipsolver->numRow());
      if (iRow > mipsolver->numRow()) {
        std::cout << "Exceed Row : " << iRow << " >= " << mipsolver->numRow() << std::endl;
        continue;
      }
      
      if (i % 2 == 0) { // lower redundant
        const bool lRedundant = isLowerRedundant(iRow);
        if (lRedundant) {
          HighsInt rstart = mipsolver->mipdata_->ARstart_[iRow];
          HighsInt rend = mipsolver->mipdata_->ARstart_[iRow + 1];
          for (auto k = rstart; k < rend; k++) {
            ...
            bool lowerNoInsert = collowerlock[iCol] + maxLockLeft < mipsolver->model_->colLoLockNum[iCol];
            bool upperNoInsert = colupperlock[iCol] + maxLockLeft < mipsolver->model_->colUpLockNum[iCol];

            if (lRedundant) {
              if (iValue > 0 && cost >= -mipsolver->options_mip_->dual_feasibility_tolerance) {
                collowerlock[iCol] ++;
                lowerNoInsert = lowerNoInsert && collowerlock[iCol] + maxLockLeft < mipsolver->model_->colLoLockNum[iCol];
              }
              else if (iValue < 0 && cost <= mipsolver->options_mip_->dual_feasibility_tolerance) {
                colupperlock[iCol] ++;
                upperNoInsert = upperNoInsert && colupperlock[iCol] + maxLockLeft < mipsolver->model_->colUpLockNum[iCol];
              }
            }
        ...
        const bool uRedundant = isUpperRedundant(iRow);
        if (uRedundant) {
          HighsInt rstart = mipsolver->mipdata_->ARstart_[iRow];
          HighsInt rend = mipsolver->mipdata_->ARstart_[iRow + 1];
          for (auto k = rstart; k < rend; k++) {
    ...
    auto checkVariableLowerLock = [&](HighsInt iCol) {
      ...
      if (model->colLoLockNum[iCol] <= model->a_matrix_.num_row_) {
        for (HighsInt k = model->a_matrix_.start_[iCol]; k < model->a_matrix_.start_[iCol + 1]; k ++) {
          ...
          const bool lhsOk = iValue > 0 && getMinActivity(iRow) >= blower - feastol();
          const bool rhsOk = iValue < 0 && getMaxActivity(iRow) <= bupper + feastol();
          if (!lhsOk && !rhsOk) {
            std::cout << "Lower lock: variable " << iCol << " at row = " << iRow << " coef = " << iValue 
                      << " not redundant at constraint " << iRow << ", minact = " << getMinActivity(iRow) << ", maxact = " << getMaxActivity(iRow) << " lhs = " << blower << " rhs = " << bupper << std::endl;
          }
        }
      }
    };
    ...
    auto checkVariableUpperLock = [&](HighsInt iCol) {
      auto model = mipsolver->model_;
      if (model->colUpLockNum[iCol] <= model->a_matrix_.num_row_) {
        for (HighsInt k = model->a_matrix_.start_[iCol]; k < model->a_matrix_.start_[iCol + 1]; k ++) {
          const HighsInt iRow = model->a_matrix_.index_[k];
          const double iValue = model->a_matrix_.value_[k];
          const double blower = model->row_lower_[iRow], bupper = model->row_upper_[iRow];
    ...
    auto addFixLower = [&](int iCol) {
      ...
      thisbchg->boundval = col_lower_[iCol];
    ...
    auto addFixUpper = [&](int iCol) {
      ...
      thisbchg->boundval = col_upper_[iCol];
      domainchangeProbing.push_back(thisbchg);
      // std::cout << "fixing to upper " << iCol << std::endl;
    };

    auto collectFixLower = [&](int iCol) {
      zeroCostFixedVariables_.emplace_back(iCol, 0);
    };

    auto collectFixUpper = [&](int iCol) {
      zeroCostFixedVariables_.emplace_back(iCol, 1);
    };


    for (auto iCol : involvedVars) {
      if (isFixed(iCol))
        continue;
      // std::cout << mipsolver->model_->colLoLockNum[iCol] << std::endl;
      // std::cout << mipsolver->model_->colUpLockNum[iCol] << std::endl;
      // for (int k = mipsolver->model_->a_matrix_.start_[iCol]; k < mipsolver->model_->a_matrix_.start_[iCol + 1]; k ++) {
      //   const int row = mipsolver->model_->a_matrix_.index_[k];
      //   const double v = mipsolver->model_->a_matrix_.value_[k];
      //   const double rlower = mipsolver->model_->row_lower_[row];
      //   const double rupper = mipsolver->model_->row_upper_[row];
      //   const double cost = mipsolver->model_->col_cost_[iCol];
      //   std::cout << iCol << " " << cost << " ";
      //   std::cout << row << " " << v << " " << rlower << " " << rupper << std::endl;
      // }
      // exit(1);
      if (cumulateReduction())
        break;
      const bool canBeFixedToLower = collowerlock[iCol] == mipsolver->model_->colLoLockNum[iCol];
      const bool canBeFixedToUpper = colupperlock[iCol] == mipsolver->model_->colUpLockNum[iCol];
      ...
      if (fabs(mipsolver->model_->col_cost_[iCol]) <= mipsolver->options_mip_->dual_feasibility_tolerance) {
        if (startZeroCostFixing) {
          if (zeroCostVarsDirection_[iCol] == 0) { // not fixed before
            if (canBeFixedToLower && canBeFixedToUpper) { // both directions are ok
              if (mipsolver->model_->col_cost_[iCol] >= 0) { // depending on cost (no tolerance)
                addFixLower(iCol);
                zeroCostVarsDirection_[iCol] = -1;
              }
              else {
                addFixUpper(iCol);
                zeroCostVarsDirection_[iCol] = 1;
            ...
            else if (canBeFixedToUpper) { // fix to upper and set its direction
              addFixUpper(iCol);
              zeroCostVarsDirection_[iCol] = 1;
            }
          }
          else if (zeroCostVarsDirection_[iCol] == 1 && canBeFixedToUpper) { // fix to upper
            addFixUpper(iCol);
          }
          else if (zeroCostVarsDirection_[iCol] == -1 && canBeFixedToLower) { // fix to lower
            addFixLower(iCol);
          }
          continue;
        }
        else { // do not perfrom zero cost variable fixing, just collect them and choose directions
          if (zeroCostVarsDirection_[iCol] == 0) { // not fixed before
            if (canBeFixedToLower && canBeFixedToUpper) { // both directions are ok
              if (mipsolver->model_->col_cost_[iCol] >= 0) { // depending on cost (no tolerance)
    ...
    // debug function
    // for (auto iCol : involvedVars) {
    //   if (collowerlock[iCol] < 0) {
    //     std::cout << "Lower : " << iCol << " " << collowerlock[iCol] << std::endl;
    //     exit(1);
    //   }
    //   if (colupperlock[iCol] < 0) {
    //     std::cout << "Upper : " << iCol << " " << colupperlock[iCol] << std::endl;
    //     exit(1);
    //   }
    // }

    clearInvolved(oldPos);

    // change bound
    HighsInt j = 0;
    for (; j != (HighsInt)domainchangeProbing.size() && !infeasible_; ++j) {
      if (!startZeroCostFixing && isFiniteBoundTightening(*domainchangeProbing[j])) {
        cumulateBchg(1);
      }
      changeBound(*domainchangeProbing[j], Reason::unspecified());

      delete domainchangeProbing[j];
    }

    // if (mipsolver->mipdata_->numRestarts == 0 && !mipsolver->submip)
    
    for (j ++; j < (HighsInt)domainchangeProbing.size(); j ++)
      delete domainchangeProbing[j];
  };
  ...
  if (!havePropagationRows() && !haveRedundantPropagationRows())
    return false;
  ...
  int nIter = 0;
  while (havePropagationRows() || haveIncreasedRedundantPropagationRows()) {
        ...
        for (HighsInt k = 0; k != numproprows; ++k) {
          ...
          if (propRowNumChangedBounds_[k].first != 0) {
            ...
            for (HighsInt j = start; j != end && !infeasible_; ++j) {
              if (!startZeroCostFixing && isFiniteBoundTightening(changedbounds[j]))
                cumulateBchg(1);
    ...
    if (haveIncreasedRedundantPropagationRows()) {
      HighsInt oldsize = redundantPropagateinds_.size();
      if (!infeasible_) {
        // update the set of redundant constraints, and the corresponding lock number of variables in them
        updateRedundantPropagation(countRedundantRow);
        nIter ++;
      } 
      countRedundantRow = oldsize;
    }
    ...
    if (dualflag && (mipsolver->options_mip_->DFProbing || mipsolver->options_mip_->GDF) && !infeasible_ && !startZeroCostFixing && !zeroCostFixedVariables_.empty()) {
      // If no more bound changes from domain propagation and dual fixing (with c_j \neq 0), we try to fix variables x_j with c_j = 0.
      // std::cout << "start zero cost fixing\n";
      startZeroCostFixing = true;
      zeroCostStartPos_ = domchgstack_.size();

      for (HighsInt j = 0; j != (HighsInt)zeroCostFixedVariables_.size() && !infeasible_; ++j) {
        const HighsInt iCol = zeroCostFixedVariables_[j].first;
        const bool isUpper = zeroCostFixedVariables_[j].second;
        if (!isFixed(iCol)) {
          if (isUpper) { // fix to upper bound
            changeBound(HighsBoundType::kLower, iCol, col_upper_[iCol], Reason::unspecified());
            if (zeroCostVarsDirection_[iCol] == 0) // set direction to avoid inconsistence
              zeroCostVarsDirection_[iCol] = 1;
          }
          else { // fix to lower bound
            changeBound(HighsBoundType::kUpper, iCol, col_lower_[iCol], Reason::unspecified());
            if (zeroCostVarsDirection_[iCol] == 0) // set direction to avoid inconsistence
              zeroCostVarsDirection_[iCol] = -1;
          } 
        }
    ...
    else if (dualflag && !infeasible_ && (mipsolver->options_mip_->DFProbing && mipsolver->options_mip_->GDF)) {
      // GDF can also eliminate some variable locks
      vector<HighsDomainChange*> domainchangeProbing;
      std::unordered_set<HighsInt> foundInds;
      vector<HighsInt>& collowerlock = tmpColLoLock_;
      vector<HighsInt>& colupperlock = tmpColUpLock_;
      for (auto x : permanentLowerLock_) {
        const HighsInt iCol = x.first;
        // skip variables with wrong direction
        if (zeroCostVarsDirection_[iCol] == 1)
          continue;
        // skip variables that can be already substituted
        if (mipsolver->mipdata_->implications.colsubstituted[iCol])
          continue;
        // skip variables with wrong-sign objective coefficients
        if (mipsolver->colCost(iCol) < -mipsolver->options_mip_->dual_feasibility_tolerance)
          continue;
        // skip fixed variables
        if (isFixed(iCol))
          continue;
        // compute the number of down-locks that can be removed by GDF
        const auto& rowSet = x.second;
        HighsInt nlock = 0;
        for (auto r : rowSet) {
          if (!redundantPropagateflags_[r])
            nlock ++;
        }
        // if all locks can be removed, we can fixed x[iCol] to its lower bound
        if (tmpColLoLock_[iCol] + nlock == mipsolver->model_->colLoLockNum[iCol]) {
          if (startZeroCostFixing) { // fix to lower
            HighsDomainChange* thisbchg = new HighsDomainChange;
            thisbchg->column = iCol;
            thisbchg->boundtype = HighsBoundType::kUpper;
            thisbchg->boundval = 0;
            domainchangeProbing.push_back(thisbchg);
            if (zeroCostVarsDirection_[iCol] == 0)
              zeroCostVarsDirection_[iCol] = -1;
          }
          else { // direction not set; we only collect x[iCol] (not actually fix it now) to avoid inconsistency
            assert(zeroCostVarsDirection_[iCol] == 0);
            zeroCostFixedVariables_.emplace_back(iCol, 0);
            zeroCostVarsDirection_[iCol] = -1;
          }
        }
      ...
      for (auto x : permanentUpperLock_) {
        const HighsInt iCol = x.first;
        // skip variables with wrong direction
        if (zeroCostVarsDirection_[iCol] == -1)
          continue;
        ...
        // compute the number of up-locks that can be removed by GDF
        const auto& rowSet = x.second;
        HighsInt nlock = 0;
        for (auto r : rowSet) {
          if (!redundantPropagateflags_[r])
        ...
        // if all locks can be removed, we can fixed x[iCol] to its upper bound
        if (tmpColUpLock_[iCol] + nlock == mipsolver->model_->colUpLockNum[iCol]) {
          if (startZeroCostFixing) { // fix to upper
            HighsDomainChange* thisbchg = new HighsDomainChange;
            thisbchg->column = iCol;
            thisbchg->boundtype = HighsBoundType::kLower;
            thisbchg->boundval = 1;
    ...
    if (infeasible_)
      countRedundantRow = redundantPropagateinds_.size();

    if (inProbing_ && dualflag && !redundantPropagateinds_.empty()) {
      std::fill(tmpColLoLock_.begin(), tmpColLoLock_.end(), 0);
      std::fill(tmpColUpLock_.begin(), tmpColUpLock_.end(), 0);
    }

    if (mipsolver->options_mip_->GDF)
      frozenRedundantResidue();
    clearRedundant();

    if (inProbing_ && nIter != 0) {
      cumulateIter(nIter);
    }
}
...
void HighsDomain::cumulateBchg(int n)
{
  if (!mipsolver->submip && inProbing_) {
    // statistics for first round
    if (mipsolver->mipdata_->num_call_probing == 0)
      mipsolver->mipdata_->num_firstround_probing_bchgs += n;
  }
}

bool HighsDomain::isFiniteBoundTightening(HighsDomainChange& boundchg)
{
  const int iCol = boundchg.column;
  if (boundchg.boundtype == HighsBoundType::kLower) {
    if (col_lower_[iCol] == -kHighsInf && boundchg.boundval != -kHighsInf)
      return false;
  }
  else {
    if (col_upper_[iCol] == kHighsInf && boundchg.boundval != kHighsInf)
      return false;
  }
  return true;
}

void HighsDomain::cumulateIter(int n)
{
  // if (!mipsolver->submip && inProbing_ && mipsolver->mipdata_->num_call_probing == 0) {
  if (!mipsolver->submip && inProbing_) {
      // std::cout << n << std::endl;
      mipsolver->mipdata_->num_firstround_probing_propagate_iter += n;
      mipsolver->mipdata_->num_firstround_probing_candidates ++;
      mipsolver->mipdata_->num_firstround_probing_max_iter = max(mipsolver->mipdata_->num_firstround_probing_max_iter, (int64_t)n);
  }
}
