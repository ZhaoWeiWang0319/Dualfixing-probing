...
void HighsMipSolver::cleanupSolve() {
  ...
  if (!timeless_log) {
    highsLogUser(options_mip_->log_options, HighsLogType::kInfo,
               ...
               "                    %.2f (presolve)\n"
               "                    %.2f (postsolve)\n"
               "                    %.2f (probing-first-round)\n",
               timer_.read(),
               timer_.read(timer_.presolve_clock),
               timer_.read(timer_.postsolve_clock),
               timer_.read(timer_.probing_clock_first_round));
    highsLogUser(options_mip_->log_options, HighsLogType::kInfo,
               "  Bchg & Fix        %llu (First-Round-Fixed-variables)\n"
               "                    %llu (First-Round-Total-variables)\n"
               "                    %llu (First-Round-Bchgs)\n"
               "                    %llu (First-Round-Probing-Propagate-Iterations-Sum)\n"
               "                    %.2f (First-Round-Probing-Propagate-Iterations-Average)\n"
               "                    %llu (First-Round-Probing-Propagate-Iterations-Max)\n",
               (long long unsigned)mipdata_->num_firstround_probing_nfix,
               (long long unsigned)mipdata_->num_firstround_probing_nTotalVars,
               (long long unsigned)mipdata_->num_firstround_probing_bchgs,
               (long long unsigned)mipdata_->num_firstround_probing_propagate_iter,
               mipdata_->num_firstround_probing_candidates == 0 ? 0 : ((double)mipdata_->num_firstround_probing_propagate_iter / mipdata_->num_firstround_probing_candidates),
               (long long unsigned)mipdata_->num_firstround_probing_max_iter);
