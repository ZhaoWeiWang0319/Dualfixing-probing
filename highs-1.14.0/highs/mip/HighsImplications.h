 ...
 public:
  ...
  /*
      Possible values for binaryInvolvedFlags_ 
       0 (0000): Not involved
       2 (0010): fixed to lower in second side probing
       1 (0001): fixed to upper in second side probing
       8 (1000): fixed to lower in first side probing
       4 (0100): fixed to upper in first side probing
       10(1010): fixed to lower in both side probing (global fixing!)
       5 (0101): fixed to upper in both side probing (global fixing!)
       9 (1001): substitutation type 1 --- x1 + x2 = 1
       6 (0110): substitutation type 2 --- x1 = x2
    */
  std::vector<uint8_t> binaryInvolvedFlags_;
  // number of these "tentative" implications containing x_j
  std::vector<uint8_t> numInvolvedImplics_;
  // used together with binaryInvolvedFlags_
  enum binaryFixType {
    kGlobalLower = 10,
    kGlobalUpper = 5,
    kSubstituteType1 = 9,
  ...
  HighsImplications(const HighsMipSolver& mipsolver) : mipsolver(mipsolver) {
    ...
    numInvolvedImplics_.assign(numcol, 0);
  }
  ...
  void clearCacheClique() {
    for (auto iCol : binaryInvolvedInds_) {
      binaryInvolvedFlags_[iCol] = 0;
      numInvolvedImplics_[iCol] = 0;
    }
