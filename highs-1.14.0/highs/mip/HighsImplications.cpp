...
bool HighsImplications::computeImplications(HighsInt col, bool val) {
  ...
  // note : in computation, sometimes we may left some redundant rows,
  //        so we need to clear it before every implication!
  globaldomain.clearRedundant();

  HighsInt stackimplicstart = domchgstack.size() + 1;
  HighsInt numImplications = -stackimplicstart;
  if (val) {
    globaldomain.setProbingStatusFlag(1);
    globaldomain.changeBound(HighsBoundType::kLower, col, 1);
  }
  else {
    globaldomain.setProbingStatusFlag(0);
  ...
  globaldomain.propagate(globaldomain.inProbing());
  globaldomain.setProbingStatusFlag(-1);

  if (isInfeasible(col, val)) return true;

  // update GDF information for the probing variable
  if (mipsolver.options_mip_->GDF && globaldomain.inProbing())
    globaldomain.probingVariableGDF(col, val);
  ...
  const bool useDFProbing = mipsolver.mipdata_->domain.inProbing() && mipsolver.options_mip_->DFProbing;  
  int tentativeStart = kHighsIInf32;
  if (useDFProbing) {
    // Mark bound changes that are deduced from applying dual fixing on variables with zero objective coefficient
    // These bound changes will not be added as single cliques or implications (i.e., R3 in Section 2.1); 
    // they can only be used to derive global reductions (i.e., R1, R2, R4 in Section 2.1, see also lines 387-462 in HighsImplications.cpp).
    // After that, they are disregarded.
    // These bound changes are marked by setting ``isTentative'' as ``true''.
    tentativeStart = globaldomain.zeroCostStartPos_;
    ...
    for (int i = 0; i < stackimplicend - stackimplicstart; i ++)
      implics_tentative[i].isTentative = (i >= tentativeStart - stackimplicstart);
  }
  ...
  for (HighsInt i = stackimplicstart; i < stackimplicend; ++i) {
    ...
    if (globaldomain.inProbing() && i >= tentativeStart) // cache tentative implications
        continue;
  ...
  if (!implics_tentative.empty()) {
    ...
    auto binstart_tmp = std::partition(implics_tentative.begin(), implics_tentative.end(),
...
bool HighsImplications::runProbing(HighsInt col, HighsInt& numReductions) {
  ...
  if (globaldomain.isBinary(col) && !implicationsCached(col, 1) &&
    ...
    // prepare for DFProbing and GDF
    globaldomain.clearOccruance();
    globaldomain.clearAllVariableTight();
    globaldomain.clearRedundantResidue();
    clearCacheClique();
    globaldomain.zeroCostStartPos_ = kHighsIInf32;
    ...
    globaldomain.updateGDFInfo();

    // Here, we analyze "tentative" bound changes of binary variables that cannot be added as cliques
    // We check if they can be used to derive global reductions.
    const bool useDFProbing = mipsolver.mipdata_->domain.inProbing() && mipsolver.options_mip_->DFProbing && !mipsolver.submip && !mipsolver.mipdata_->cliquetable.isFull();
      ...
      do
      {
        ...
        for (auto k : binaryInvolvedInds_) {
          ...
          if (data == binaryFixType::kGlobalLower) { // x[k] is fixed at 0 under both x[col] = 0 and x[col] = 1
            ...
            data = 0;
            haveReduction = true;    
            globaldomain.cumulateBchg(numInvolvedImplics_[k]);        
          }
          ...
          else if (data == binaryFixType::kSubstituteType1) { // x[k] is fixed at 0 under x[col] = 1, and is fixed at 1 under x[col] = 0; this makes x[col] + x[k] = 1
          ...
          else if (data == binaryFixType::kSubstituteType2) { // x[k] is fixed at 0 under x[col] = 0, and is fixed at 1 under x[col] = 1; this makes x[col] = x[k]
            // Adding two cliques (i.e., these two cliques should be added in computeImplications() to derive global reductions)
            clique[0] = HighsCliqueTable::CliqueVar(col, 1);
            clique[1] = HighsCliqueTable::CliqueVar(k, 0);
            cliquetable.addClique(mipsolver, &clique[0], 2);
    ...
    // Also include the bound changes of non-binary variables here, to derive tighter global bounds and variable substitutions
    const bool haveTentativeImplics_zero = !implications[2 * col].implics_tentative.empty();
    const bool haveTentativeImplics_one  = !implications[2 * col + 1].implics_tentative.empty();


    // analyze implications
    const std::vector<HighsDomainChange>& implicsdown =
        haveTentativeImplics_zero ? getImplications_tentative(col, 0) : getImplications(col, 0, infeasible);
    const std::vector<HighsDomainChange>& implicsup =
        haveTentativeImplics_one ? getImplications_tentative(col, 1) : getImplications(col, 1, infeasible);
    ...
    while (u < nimplicsup && d < nimplicsdown) {
      if (implicsup[u].column < implicsdown[d].column)
        ...
        int bestd_lb = -1, bestd_ub = -1, bestu_lb = -1, bestu_ub = -1;
         do {
          if (implicsdown[d].boundtype == HighsBoundType::kLower) {
            if (lbDown < implicsdown[d].boundval) {
              lbDown = implicsdown[d].boundval;
              bestd_lb = d;
            }
            // lbDown = std::max(lbDown, implicsdown[d].boundval);
          }
          else {
            if (ubDown > implicsdown[d].boundval) {
              ubDown = implicsdown[d].boundval;
              bestd_ub = d;
            }
        ...
        do {
          if (implicsup[u].boundtype == HighsBoundType::kLower) {
            if (lbUp < implicsup[u].boundval) {
              lbUp = implicsup[u].boundval;
              bestu_lb = u;
            }
            // lbUp = std::max(lbUp, implicsup[u].boundval);
          }
          else {
            if (ubUp > implicsup[u].boundval) {
              ubUp = implicsup[u].boundval;
              bestu_ub = u;
            }
        ...
        if (lbDown == ubDown && lbUp == ubUp &&
          ...
          ++numReductions;
          globaldomain.cumulateBchg(bestd_lb == -1 ? 0 : numInvolvedImplics_[implicsdown[bestd_lb].column]);
          globaldomain.cumulateBchg(bestd_ub == -1 ? 0 : numInvolvedImplics_[implicsdown[bestd_ub].column]);
          globaldomain.cumulateBchg(bestu_lb == -1 ? 0 : numInvolvedImplics_[implicsup[bestu_lb].column]);
          globaldomain.cumulateBchg(bestu_ub == -1 ? 0 : numInvolvedImplics_[implicsup[bestu_ub].column]);
        } else {
          int nImplics_lb = 0, nImplics_ub = 0; 
          double lb, ub;
          if (lbDown <= lbUp) {
            lb = lbDown;
            nImplics_lb += bestd_lb == -1 ? 0 : numInvolvedImplics_[implicsdown[bestd_lb].column];
          }
          else {
            lb = lbUp;
            nImplics_lb += bestu_lb == -1 ? 0 : numInvolvedImplics_[implicsup[bestu_lb].column];
          }

          if (ubDown >= ubUp) {
            ub = ubDown;
            nImplics_ub += bestd_ub == -1 ? 0 : numInvolvedImplics_[implicsdown[bestd_ub].column];
          }
          else {
            ub = ubUp;
            nImplics_ub += bestu_ub == -1 ? 0 : numInvolvedImplics_[implicsup[bestu_ub].column];
          }
          ...
          if (lb > globaldomain.col_lower_[implcol]) {
            ...
            ++numReductions;
            globaldomain.cumulateBchg(nImplics_lb);
          }
          ...
          if (ub < globaldomain.col_upper_[implcol]) {
            globaldomain.changeBound(HighsBoundType::kUpper, implcol, ub,
                                     HighsDomain::Reason::unspecified());
            ++numReductions;
            globaldomain.cumulateBchg(nImplics_ub);
          }
        }
      }
    }
...
void HighsImplications::cacheTmpCliques(bool val, const HighsDomainChange& bchg) {
  ...
  if (val == 0) { // probing x_k = 0
    if (bchg.boundtype == HighsBoundType::kLower) { // fixed to 1
      if (!isFixedTo1(val, iCol)) {
        ...
        binaryInvolvedFlags_[iCol] += 1; // 0001
        numInvolvedImplics_[iCol] += bchg.isTentative;
      }
    }
    else { // fixed to 0
      if (!isFixedTo0(val, iCol)) {
        if (binaryInvolvedFlags_[iCol] == 0)
          binaryInvolvedInds_.push_back(iCol);
        binaryInvolvedFlags_[iCol] += 2; // 0010
        numInvolvedImplics_[iCol] += bchg.isTentative;
      }
    }
  }
    ...
    if (bchg.boundtype == HighsBoundType::kLower) { // fixed to 1
      if (!isFixedTo1(val, iCol)) {
        if (binaryInvolvedFlags_[iCol] == 0)
          binaryInvolvedInds_.push_back(iCol);
        binaryInvolvedFlags_[iCol] += 4; // 0100
        numInvolvedImplics_[iCol] += bchg.isTentative;
      }
    }
    else { // fixed to 0
      if (!isFixedTo0(val, iCol)) {
        if (binaryInvolvedFlags_[iCol] == 0)
          binaryInvolvedInds_.push_back(iCol);
        binaryInvolvedFlags_[iCol] += 8; // 1000
        numInvolvedImplics_[iCol] += bchg.isTentative;
      }
    }
  }
